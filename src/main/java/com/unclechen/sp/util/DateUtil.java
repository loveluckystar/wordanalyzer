...
public class DateUtil {
  ...
  static final String dayNames[] = { "星期日", "星期一", "星期二", "星期三", "星期四", "星期五",
          "星期六" };
  ...
  public static boolean isExpire(String date) {
    ...
    int datenum =Integer.parseInt(date.replaceAll("-",""));
    int datenumcurrent =Integer.parseInt(dateToString(new Date(), "yyyyMMdd"));
    return (datenumcurrent>datenum);
  }
  ...
  public static long timeSub(Date firseTime,Date secTime)
  {
    return (secTime.getTime()-firseTime.getTime())/1000;
  }
  ...
  //计算今天到期限日的天数
  public static long daysFromToday(String str){
    if(str == null || "".equals(str)) return -1;
    Date date = stringtoDate(str,LONG_DATE_FORMAT);
    Date today = stringtoDate(currDay(),LONG_DATE_FORMAT);
  ...
  public static String getCurrent() {
    ...
    sb.append(year).append("_").append(StringUtil.addzero(month, 2))
            .append("_").append(StringUtil.addzero(day, 2)).append("_").append(
            StringUtil.addzero(hour, 2)).append("_").append(
  ...
  public static String getAstro(String birth) {
    ...
    int month = Integer.parseInt(birth.substring(birth.indexOf("-") + 1, birth
            .lastIndexOf("-")));
   ...
   *          相加(相减)的月数
   */
  public static  String nextMonthString(int months) {
    Calendar cal = Calendar.getInstance();
    cal.add(Calendar.MONTH, months);
    return cal.get(Calendar.YEAR)+""+StringUtil.addzero(cal.get(Calendar.MONTH)+1,2);
  }
  ...
  public static boolean isWeekday(String dates){
    SimpleDateFormat sdfInput = new SimpleDateFormat("yyyy-MM-dd");
    Calendar calendar = Calendar.getInstance();
    Date date = new Date();
    try {
   ...
   * @return
   */
  public static List<String> getDateList(int num, String format){
    List<String> list = new ArrayList<String>();
    if(num <=0){
      list.add(DateUtil.getCurrDate(format));
      return list;
    }
    Calendar cal = Calendar.getInstance();
    ...
    for(int i=-num;i<num;i++){
      cal.add(Calendar.MONTH, 1);
      String date = dateToString(cal.getTime(), format);
      list.add(date);
    }
    return list;
  }
  ...
  public static String getDateBySeconds(String seconds){
    ...
    long n = 0;
    try{
      n = Long.parseLong(seconds);
    }catch(Exception e){
      e.printStackTrace();
    }
    calendar.setTimeInMillis(n*1000);
    return (DateUtil.dateToString(calendar.getTime(), DateUtil.FORMAT_ONE));
  }
  ...
  public static long getSecondsByDate(String date){
    Calendar calendar = Calendar.getInstance();
    calendar.setTime(stringtoDate(date,DateUtil.LONG_DATE_FORMAT));
